<head>
	
	<meta http-equiv="X-UA-Compatible" content="IE=Edge">
	<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1"> 
	
	<title>Gedai - Gerenciador de Demandas e Atividades Internas</title>
	<link href="resources/img/favicon.ico" rel="shortcut icon">
	<link type="text/css" rel="stylesheet" href="resources/css/style.css?${version}">
	<link type="text/css" rel="stylesheet" href="resources/css/semantic.min.css">
	<link type="text/css" rel="stylesheet" href="resources/css/objects.css?${version}">
	<link type="text/css" rel="stylesheet" href="resources/css/alertify.core.css" />
	<link type="text/css" rel="stylesheet" href="resources/css/alertify.default.css" />
	<link type="text/css" rel="stylesheet" href="resources/css/animate.css" />
	
	<script type="text/javascript" src="resources/js/jquery-2.1.4.jsx"></script>
	<script type="text/javascript" src="resources/js/jquery.tmpl.min.jsx"></script>
	<script type="text/javascript" src="resources/js/jquery.tmplPlus.jsx"></script>
	<script type="text/javascript" src="resources/js/semantic.min.jsx"></script>
	<script type="text/javascript" src="resources/js/alertify.min.jsx"></script>
	<script type="text/javascript" src="resources/js/functions.jsx?${version}"></script>
	<script type="text/javascript" src="resources/js/jquery.mask.min.jsx"></script>
	<script type="text/javascript" src="resources/js/DateUtils.jsx?${version}"></script>
	<script type="text/javascript" src="resources/js/jquery.fileDownload.jsx"></script>
	
</head>
